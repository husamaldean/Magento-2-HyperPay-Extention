...
class Data extends \Magento\Framework\App\Helper\AbstractHelper
{
    ...
    {  
        try{
           /* if(!($this->_checkoutSession->getQuote())) {
                $this->doError('Order id is not found');
            }   
     ...
     * @return double
     */  
    public function convertPrice($code,$amountValue)
    {   
        $currentCurrency = $this->_adapter->getSupportedCurrencyCode($code);
	...
	*/
    ...
    public function getBillingAndShippingAddress($order)
    {
        ...
        if (isset($shippingAddress) && !empty($shippingAddress)) {
            $firstNameShipping = str_replace("&", "", $order->getShippingAddress()->getFirstname());
            $surNameShipping = str_replace("&", "", $order->getShippingAddress()->getLastname());
            ...
            $streetShipping = str_replace("&", "", $order->getShippingAddress()->getStreet());
            $cityShipping = str_replace("&", "", $order->getShippingAddress()->getCity());
        ...
        $firsName = str_replace("&", "", $order->getBillingAddress()->getFirstname());
        $surName = str_replace("&", "", $order->getBillingAddress()->getLastname());
        $country = $order->getBillingAddress()->getCountryId();
        $tel = $order->getBillingAddress()->getTelephone();
        $postCode = $order->getBillingAddress()->getPostcode();
        $state = $order->getBillingAddress()->getRegion();
        $street = str_replace("&", "", $order->getBillingAddress()->getStreet());
        $city = str_replace("&", "", $order->getBillingAddress()->getCity());
